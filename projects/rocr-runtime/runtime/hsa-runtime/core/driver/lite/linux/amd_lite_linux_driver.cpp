...
hsa_status_t LinuxAmdgpuLiteDriver::AllocateMemory(
    ...
    size_t size, uint32_t, core::DriverMemoryHandle* handle) {
  ...
  *handle = {};
  ...
  // New DriverMemoryHandle ABI: the runtime keys allocation_map_ on handle.vaddr
  // and recovers the pointer from handle.handle in FreeMemory. Mirror KfdDriver:
  // handle.handle == the allocation's CPU pointer (also the free/lookup key).
...
hsa_status_t LinuxAmdgpuLiteDriver::FreeMemory(const core::DriverMemoryHandle& handle) {
  return FreeMemoryPtr(reinterpret_cast<void*>(handle.handle));
}

hsa_status_t LinuxAmdgpuLiteDriver::FreeMemoryPtr(void* mem) {
...
hsa_status_t LinuxAmdgpuLiteDriver::ExportMemoryHandle(
    const core::Agent&, const core::DriverMemoryHandle&, core::ShareType,
    void*) {
  return HSA_STATUS_ERROR;
}

hsa_status_t LinuxAmdgpuLiteDriver::ImportMemoryHandle(
    const core::Agent&, core::DriverMemoryHandle*, core::ShareType, void*,
    void*) {
  return HSA_STATUS_ERROR;
}

hsa_status_t LinuxAmdgpuLiteDriver::Map(const core::DriverMemoryHandle&, void*,
                                        size_t, size_t, hsa_access_permission_t,
                                        uint32_t) {
  return HSA_STATUS_ERROR;
}

hsa_status_t LinuxAmdgpuLiteDriver::Unmap(const core::DriverMemoryHandle&,
                                          void*, size_t, size_t, uint32_t) {
  return HSA_STATUS_ERROR;
}

hsa_status_t LinuxAmdgpuLiteDriver::CreateShareableHandle(
    core::DriverMemoryHandle*, const core::Agent&, uint64_t*) {
  return HSA_STATUS_ERROR;
}

hsa_status_t LinuxAmdgpuLiteDriver::DestroyMemoryHandle(
    core::DriverMemoryHandle*) {
  return HSA_STATUS_ERROR;
}

hsa_status_t LinuxAmdgpuLiteDriver::GetDeviceFd(uint32_t, int* fd) const {
  // amdgpu_lite exposes no per-node DRM render fd to the runtime.
  if (fd) *fd = -1;
  return HSA_STATUS_ERROR;
}

hsa_status_t LinuxAmdgpuLiteDriver::CheckAcceleratorReadiness(core::Agent&,
                                                              bool* ready) const {
  // gfx1201 is brought up out-of-band (LITE_MES_RECIPE); by the time the
  // runtime queries readiness the agent is usable.
  if (ready) *ready = true;
  return HSA_STATUS_SUCCESS;
}
...
hsa_status_t LinuxAmdgpuLiteDriver::MakeMemoryResident(
    const void* mem, size_t, uint64_t* alternate_va, const HsaMemFlags*,
